...
class UserCanSearchAUserLikesTest extends TestCase
{
    ...
    public function user_dont_need_to_be_loged_in_to_search_an_user_likes()
    {
        ...
        $this->getJson("api/v1/likes?user={$user->id}")->assertStatus(Response::HTTP_OK);
    ...
    public function search_other_user_likes()
    {
        ...
        $response = $this->getJson("api/v1/likes?user={$user->id}&s=random thou");
        ...
        $response = $this->getJson("api/v1/likes?user={$user->id}");
        $response->assertStatus(Response::HTTP_OK);
    ...
    public function search_its_own_likes()
    {
        ...
        $response = $this->actingAs($user, 'api')->getJson("api/v1/likes?s=random thou");
        $response->assertStatus(Response::HTTP_OK);
        $this->assertCount(1, $response->json()['data']);
        $response->assertJson([
            'data' => [
        ...
        $response = $this->actingAs($user, 'api')->getJson("api/v1/likes");
        $response->assertStatus(Response::HTTP_OK);
        $this->assertCount(11, $response->json()['data']);
    ...
    public function returns_404_when_user_is_not_loged_in_and_no_user_is_provided_in_query_string()
    {
        ...
        $this->withExceptionHandling()->getJson('api/v1/likes')->assertStatus(Response::HTTP_NOT_FOUND);
